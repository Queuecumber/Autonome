...
from graphiti_mcp import server
...
def graph(monkeypatch, graph_backend):
    ...
    import graphiti_mcp.model as model
...
async def test_empty_batches_remain_noops(graph, monkeypatch, provenance):
    ...
    assert await graph.save_memory(relationships=[], **provenance) == graph.MemorySaved(
...
async def test_story_keeps_narrative_source_and_dates_separate(graph):
    ...
    assert story.valid_at == when
    assert story.recorded_at > when
...
async def test_reusing_an_episode_rejects_conflicting_provenance(graph, provenance):
    ...
    assert await graph.list_relationships() == graph.RelationshipPage(relationships=[], next_cursor=None)
...
async def test_a_backdated_fact_keeps_the_date_it_was_given(graph):
    ...
    r = await graph.save_memory(
        ...
        valid_at=when)
    assert r.relationships[0].valid_at.date() == when.date()
...
async def test_each_fact_can_keep_its_own_historical_date(graph):
    ...
    assert [f.valid_at for f in result.relationships] == [
        april, july, fallback]
    assert all(f.recorded_at > july for f in result.relationships)
...
async def test_omitted_date_still_defaults_to_the_recording_time(graph, monkeypatch):
    ...
    assert result.relationships[0].valid_at == result.relationships[0].recorded_at == when
...
async def test_fact_dates_survive_mcp_tool_validation(graph):
    ...
    assert [fact["valid_at"] for fact in relationships] == [
        "2026-04-10T00:00:00Z", None, "2026-06-01T00:00:00Z"]
...
async def test_date_correction_retains_the_fact_story_and_embedding(embedded):
    ...
    assert (await embedded.search(kind="relationships", query=text))[0].valid_at == when
...
async def test_date_correction_cannot_invert_a_facts_validity(graph):
    ...
    assert stored.valid_at == original_date
    assert not stored.valid_at_corrections
...
async def test_inventory_pages_through_all_facts_including_history(graph, monkeypatch):
    ...
    assert await graph.list_relationships(cursor=expected[-1]) == graph.RelationshipPage(
...
def test_evidence_kind_rejects_unrecognized_certainty_labels():
    ...
    from graphiti_mcp.model import Relationship
...
async def test_entity_inventory_pages_actual_entities_including_isolated_ones(graph, monkeypatch):
    ...
    assert first.entities[0] == graph.Entity(
    ...
    assert await graph.list_entities(cursor=identifiers[-1]) == graph.EntityPage(
...
async def test_entity_inventory_is_empty_for_an_empty_graph(graph):
    ...
    assert await graph.list_entities() == graph.EntityPage(entities=[], next_cursor=None)
...
async def test_entity_search_returns_names_with_bounded_relationship_previews(graph):
    ...
    assert isinstance(entity, graph.EntityMatch)
...
async def test_entity_explanation_handles_missing_isolated_and_truncated_results(graph):
    ...
    assert await graph.explain_entity("missing") == graph.EntityExplanation(found=False, name="missing")
...
async def test_mcp_terminology_schema_and_entity_workflow(graph):
    ...
    assert "The *story* is the memory" in server.mcp.instructions
    assert "feelings" in server.mcp.instructions and "not objective fact" in server.mcp.instructions
    documented = re.findall(r"`([a-z_]+)(?:\([^`]*\))?`", server.mcp.instructions)
    parameters = {"valid_at", "recorded_at", "truncated", "source", "story_id", "uri"}
    assert set(documented) - parameters <= set(tools)


@pytest.mark.asyncio
async def test_model_returns_typed_records_all_the_way_through(graph):
    """Domain operations expose records and native dates, including every nested result."""
    saved = await graph.save_memory(relationships=[graph.Relationship(
        subject="A", relation="RECALLS", object="B", description="A recalls B fondly",
    )], story="The full narrative", source="message/123")
    assert isinstance(saved, graph.MemorySaved)
    relationship = saved.relationships[0]
    assert isinstance(relationship, graph.RelationshipRecord)
    assert isinstance(relationship.recorded_at, datetime)
    assert isinstance(relationship.valid_at, datetime)
    assert relationship.valid_at_corrections == [] and not relationship.superseded
    assert saved.story_uri == relationship.story_uri

    corrected = await graph.set_valid_at(relationship.relationship_id, None, reason="Unknown date")
    assert isinstance(corrected.valid_at_corrections[0], graph.DateCorrection)
    assert isinstance(corrected.valid_at_corrections[0].corrected_at, datetime)
    assert corrected.valid_at is None
    story = await graph.get_story(saved.story_id)
    assert isinstance(story, graph.Story) and story.uri == saved.story_uri
    assert isinstance(story.recorded_at, datetime) and not story.story_truncated
    ...
    assert isinstance(preview, graph.EntityMatch)
    assert isinstance(preview.relationships[0], graph.RelationshipRecord)
    assert isinstance(await graph.list_entities(), graph.EntityPage)
    assert isinstance(await graph.list_relationships(), graph.RelationshipPage)
    explored = await graph.explore("A")
    assert isinstance(explored, graph.Exploration) and isinstance(explored.entities[0], graph.EntityVisit)
    path = await graph.find_path("B", "A")
    assert isinstance(path, graph.Path) and isinstance(path.relationships[0], graph.PathStep)
    assert path.relationships[0].subject == "A" and not path.relationships[0].traversed_forward
    explained = await graph.explain_entity("A")
    assert isinstance(explained, graph.EntityExplanation)
    assert isinstance(explained.relationships[0], graph.RelationshipExplanation)
    assert isinstance(explained.relationships[0].sources[0], graph.Story)


@pytest.mark.asyncio
async def test_typed_graph_resources_round_trip_through_mcp(graph):
    """Resources automatically serialize the same model objects as the tool layer."""
    from fastmcp import Client

    saved = await graph.save_memory(relationships=[graph.Relationship(
        subject="A", relation="KNOWS", object="B", description="A knows B",
    )], story="Complete narrative", source="source/123")
    relationship = saved.relationships[0]
    async with Client(server.mcp) as client:
        ...
        for uri, expected, result_type in [
            ...
            decoded = result_type.model_validate_json(contents[0].text)
            assert decoded == expected
            assert "fact_embedding" not in contents[0].text


@pytest.mark.asyncio
async def test_resource_reads_and_story_reuse_enforce_memory_group(graph, monkeypatch):
    """Known foreign IDs cannot leak resources or attach private stories to a new memory."""
    from fastmcp import Client
    from mcp.shared.exceptions import McpError

    saved = await graph.save_memory(relationships=[graph.Relationship(
        subject="Private", relation="KNOWS", object="Secret", description="Private knows Secret",
    )], story="Private narrative")
    relationship = saved.relationships[0]
    foreign_edge = await graph.store.get_edge(relationship.relationship_id)
    monkeypatch.setattr(graph.store, "GROUP_ID", "other")
    for read, identifier in [
        (graph.read_entity, relationship.subject_id),
        (graph.read_relationship, relationship.relationship_id),
        (graph.get_story, saved.story_id),
    ]:
        with pytest.raises(ValueError, match="group"):
            await read(identifier)
    with pytest.raises(ValueError, match="group"):
        await graph._relationship_record(foreign_edge)
    with pytest.raises(ValueError, match="group"):
        await graph.save_memory(relationships=[graph.Relationship(
            subject="Other", relation="ADDS", object="Memory", description="Other adds Memory",
...
@pytest.mark.asyncio
@pytest.mark.parametrize("method", ["read_entity", "read_relationship", "get_story"])
async def test_resource_reads_reject_missing_or_malformed_ids(graph, method):
    """Malformed resource identifiers and missing records raise rather than inventing data."""
    from graphiti_core.errors import EdgeNotFoundError, NodeNotFoundError

    read = getattr(graph, method)
    with pytest.raises(ValueError):
        await read("not-a-uuid")
    with pytest.raises((EdgeNotFoundError, NodeNotFoundError)):
        await read(str(uuid.uuid4()))


@pytest.mark.asyncio
async def test_all_mcp_tools_advertise_and_obey_typed_output_schemas(graph):
    """Every action's actual JSON output validates against its generated output schema."""
    from fastmcp import Client
    from jsonschema import Draft202012Validator

    saved = await graph.save_memory(relationships=[graph.Relationship(
        subject="A", relation="KNOWS", object="B", description="A knows B",
    )], story="Source narrative")
    relationship_id = saved.relationships[0].relationship_id
    missing_id = str(uuid.uuid4())
    cases = [
        ("save_memory", {"relationships": []}),
        ("search", {"query": "A"}),
        ("search", {"query": "A", "kind": "relationships"}),
        ("search", {"query": "missing"}),
        ("list_entities", {}), ("list_relationships", {}),
        ("explore", {"name": "A"}), ("explore", {"name": "missing"}),
        ("find_path", {"source": "A", "target": "B"}),
        ("find_path", {"source": "A", "target": "missing"}),
        ("explain_relationship", {"relationship_id": relationship_id}),
        ("explain_relationship", {"relationship_id": missing_id}),
        ("explain_entity", {"name": "A"}), ("explain_entity", {"name": "missing"}),
        ("get_entity", {"name": "A"}), ("get_entity", {"name": "missing"}),
        ("get_story", {"story_id": saved.story_id}), ("list_vocabulary", {}),
        ("set_valid_at", {"relationship_id": relationship_id, "valid_at": None}),
        ("supersede", {"relationship_id": relationship_id}),
    ]
    async with Client(server.mcp) as client:
        tools = {tool.name: tool for tool in await client.list_tools()}
        assert set(tools) == {name for name, _ in cases}
        ...
        properties = tools["save_memory"].outputSchema["properties"]["relationships"]["items"]["properties"]
        assert properties["recorded_at"]["format"] == "date-time"
        correction = properties["valid_at_corrections"]["items"]["properties"]
        assert correction["corrected_at"]["format"] == "date-time"


def test_graph_model_is_independent_of_mcp_and_server_does_no_result_coercion():
    """The model owns data flow; the view only registers typed actions and resources."""
    import ast
    import inspect
    from graphiti_mcp import model

    model_tree = ast.parse(inspect.getsource(model))
    for node in ast.walk(model_tree):
        if isinstance(node, ast.ImportFrom):
            assert not node.module.startswith(("fastmcp", "mcp"))
        if isinstance(node, ast.Import):
            assert not any(alias.name.startswith(("fastmcp", "mcp")) for alias in node.names)
    server_tree = ast.parse(inspect.getsource(server))
    assert not any(isinstance(node, ast.Attribute) and node.attr in {
        "model_dump", "model_dump_json", "dict", "dumps", "execute_query",
